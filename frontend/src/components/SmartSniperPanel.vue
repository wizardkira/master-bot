<template>
  <article class="rounded border border-slate-800 bg-slate-900/60 p-4">
    <header class="mb-3 flex items-center justify-between">
      <h3 class="font-semibold">Smart Sniper</h3>
      <span class="text-xs" :class="statusClass">{{ status }}</span>
    </header>
    <p class="mb-4 text-sm text-slate-400">Discovery + análise profunda + execução com KPI dinâmico.</p>
...
const status = computed(() => botStore.smartSniperStatus);
const statusClass = computed(() => (status.value === 'RUNNING' ? 'text-emerald-400' : 'text-rose-400'));

const start = async () => botStore.startMode('smart-sniper');
const stop = async () => botStore.stopMode('smart-sniper');
</script>
